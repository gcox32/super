export default class LdrsBaseElement extends HTMLElement {
    ...
    storePropsToUpgrade(props: string[]): void {
      props.forEach((prop) => {
        if (this.hasOwnProperty(prop) && typeof (this as any)[prop] !== 'undefined') {
          (this._propsToUpgrade as any)[prop] = (this as any)[prop]
    ...
    applyDefaultProps(props: object): void {
      ...
      Object.entries(props).forEach(([key, defaultValue]) => {
        (this as any)[key] = (this as any)[key] || defaultValue.toString()
      })
    }
  }